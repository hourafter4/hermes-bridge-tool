...
from hermes_bridge.config import Settings, load_settings
from hermes_bridge.setup import pair_server, pairing_command, register_clients, run_setup
...
class SetupTests(unittest.TestCase):
    def setUp(self):
        ...
        environment = patch.dict(os.environ, {"HERMES_BRIDGE_CONFIG": str(self.config)})
        environment.start()
    ...
    def test_failed_pairing_leaves_local_files_untouched(self):
        ...
        for result in failures:
            with self.subTest(result=result.returncode), patch("hermes_bridge.setup.subprocess.run", return_value=result), self.assertRaises(ValueError) as error:
    ...
    def test_timeout_preserves_local_state_and_reports_uncertain_remote_result(self):
        with patch("hermes_bridge.setup.subprocess.run", side_effect=subprocess.TimeoutExpired("ssh", 120, output="secret")), self.assertRaisesRegex(ValueError, "may have changed") as error:
            pair_server(self.settings)
    ...
    def test_config_write_failure_restores_previous_key(self):
        ...
        with patch("hermes_bridge.setup.subprocess.run", return_value=result), patch("hermes_bridge.setup.save_settings", side_effect=PermissionError("read only")), self.assertRaises(PermissionError):
            pair_server(self.settings)
        self.assertEqual(self.key.read_text(), "original\n")
        self.assertFalse(self.config.exists())
    ...
    def test_register_preflights_all_clients_and_uses_argument_arrays(self):
        with patch("hermes_bridge.setup.find_command", side_effect=lambda name: "/bin/codex" if name == "codex" else None), patch("hermes_bridge.setup.subprocess.run") as run, self.assertRaises(ValueError):
            register_clients("both")
        run.assert_not_called()
        with patch("hermes_bridge.setup.find_command", side_effect=lambda name: "/bin/" + name), patch("hermes_bridge.setup.bridge_command", return_value=["/path with spaces/hermes-bridge", "mcp"]), patch("hermes_bridge.setup.subprocess.run", return_value=self.completed({})) as run:
            self.assertEqual(register_clients("both"), ["codex", "claude"])
        self.assertEqual(run.call_args_list[0].args[0][-2:], ["/path with spaces/hermes-bridge", "mcp"])
    ...
    def test_claude_repeat_registration_is_idempotent_without_removing_entry(self):
        duplicate = subprocess.CompletedProcess([], 1, "", "MCP server hermes already exists in user config")
        existing = subprocess.CompletedProcess([], 0, "hermes:\n  Scope: User config (available in all your projects)\n  Command: /bin/hermes-bridge\n  Args: mcp\n", "")
        with patch("hermes_bridge.setup.find_command", return_value="/bin/claude"), patch("hermes_bridge.setup.bridge_command", return_value=["/bin/hermes-bridge", "mcp"]), patch("hermes_bridge.setup.subprocess.run", side_effect=[duplicate, existing]) as run:
            self.assertEqual(register_clients("claude"), ["claude"])
        self.assertEqual(run.call_args_list[-1].args[0][1:], ["mcp", "get", "hermes"])
    ...
    def test_noninteractive_setup_preflights_without_remote_changes(self):
        ...
        with patch("hermes_bridge.setup.pair_server") as pair, self.assertRaises(ValueError):
            run_setup(args)
    ...
    def test_wizard_completion_does_not_print_pairing_key(self):
        ...
        output = io.StringIO()
        with patch("hermes_bridge.setup.pair_server", return_value={"ready": True}) as pair, patch("hermes_bridge.setup.register_clients", return_value=[]), patch("sys.platform", "linux"), contextlib.redirect_stdout(output):
